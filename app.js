...
function getProductKeyFromUrl() {
     ...
     * Priority:
     * 1. ?product=... in URL
     * 2. window.NSH_PRODUCT_ID defined by the application
     * 3. template as final fallback
     */
    ...
    if (urlProduct) {
        return urlProduct.trim().toLowerCase();
    }

    if (
        typeof window.NSH_PRODUCT_ID === "string" &&
        window.NSH_PRODUCT_ID.trim() !== ""
    ) {
        return window.NSH_PRODUCT_ID
            .trim()
            .toLowerCase();
    }
